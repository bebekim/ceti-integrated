function P=load_defalt_parameters(U_T,constarint_flag,SNR_thresh,Coda_count_threshold)

%% Parameters settings:
    P.creaks_flag=1;              % Optional: set 1 to ignore potential creaks | set 0 for normal operation
    P.pulse_width=1;              % Define pulse width in [ms]
    P.U_T=U_T;                    % Coda Detection threshold
    ...
    P.CDF_test_threshold=0.6;     % Threshold to filter out non-coda clicks from subsequent analysis
    P.freq_test_idx=414;          % Frequency index of the CDF threshold point
    ...
    P.Coda_count_threshold=Coda_count_threshold;            % Coda presence threshold - declare the presence of codas in a buffer if the number of detected codas exceeds this threshold 
    P.constarint_flag=constarint_flag;                      % Apply contraints: set 1 for Yes | set 0 for No
