...
%% Parameters settings:
    ...
    if Tag_flag
        P.snr_lim=50e-3;      % minimum abs amplitude threshold 
        P.dist_min=28e-3;       % minimum distance between consecutive clicks                
    else
    ...
    P.SNR_thresh=65;%52;        % Min SNR threshold [in dB]
    P.D_Threshold=1.42;         % set the click presence detection threshold
    load bp_filter_sos.mat;     % load BPF (2-24khz) parameters (sos and g)
    P.sos=sos;
    P.g=g;
