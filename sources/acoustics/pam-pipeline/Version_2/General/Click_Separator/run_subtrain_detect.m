...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while(1)               
        ...
        Clicks_within_buffer.Slant_delays=Click_detections.Slant_delays(buffer_indices);
        ...
        if Plot_flag.Click_separation
            ...
            for i=1:length(Detected_subtrains)
                ind=Detected_subtrains{i};              
                dotH=plot(Clicks_within_buffer.ToAs(ind),Clicks_within_buffer.Slant_delays(ind),'*','LineWidth',2); hold on; 
                   pause(0.05);  % calls DRAWNOW implicitly
                   set(dotH, 'XData', Clicks_within_buffer.ToAs(ind), 'YData', Clicks_within_buffer.Slant_delays(ind)); hold on;
            end
        ...
        for i=1:length(Detected_subtrains)
            ...
            Detections(t_index).ref(i)={Clicks_within_buffer.Slant_delays(ind)};
            Detections(t_index).ICI(i)={diff(Clicks_within_buffer.ToAs(ind))};
            Detections(t_index).wav_avg(i)={Waveform_average(Clicks_within_buffer.ToAs(ind),Fs,Sig,0)};
        end         
    end
