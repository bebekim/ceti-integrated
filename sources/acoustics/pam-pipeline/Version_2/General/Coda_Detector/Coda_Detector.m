function Separated_codas=Coda_Detector(Y_filtered,Fs,P,Click_detections,Separated_Clicks,Plot_flag)
        ...
        % Y_filtered=S_t_filtered;
        % P=Coda_detector_settings;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if length(Echolocation_clicks)/(length(Y_filtered)/Fs)<0.3
        Echolocation_clicks.ToAs=[];
        Echolocation_clicks.Amps=[];
    end
    ...
    for i=1:length(Non_echolocation_clicks.ToAs)
        CDF_est=Spectral_features_extraction(Y_filtered,Fs,Fs*Non_echolocation_clicks.ToAs(i),P);
        if CDF_est(P.freq_right_idx)>P.CDF_right_threshold && CDF_est(P.freq_left_idx)<P.CDF_left_threshold
    ...
    while(int32((In+P.Buffer_size)*Fs)<length(Y_filtered))
        ...
        Locs=Coda_candidates.ToAs(Coda_candidates.ToAs>t1 & Coda_candidates.ToAs<t2);
        Pks=Coda_candidates.Amps(Coda_candidates.ToAs>t1 & Coda_candidates.ToAs<t2);               
    ...
    if Coda_count>P.Coda_count_threshold
        ...
        [Separated_codas,~]=Codas_Separation(Fs,U_max_all,P.U_T,All_codas,Y_filtered,t_filtered,0); % Plot coda detections
        if ~isempty(Separated_codas)
            % Separated_codas=Remove_mutipath(Separated_codas,Y_filtered,Fs,Plot_flag.Coda_detection);
            Separated_codas=Merge_overlapped_codas(Separated_codas,Y_filtered,Fs,Plot_flag.Coda_detection);
        end
        ICI_vec=[];
        for i=1:length(Separated_codas)
           ICI_vec=[ICI_vec diff(Separated_codas{i})];
        end

        if mean(ICI_vec)>P.Coda_ICI_thresh_max && std(ICI_vec)< P.Coda_ICI_thresh_min
              if Plot_flag.Coda_detection
                  close;
